...
const selectChat = (chat) => {
  ...
  chatStore.activeTab = 'chat';  // 채팅 탭으로 전환
};
...
<template>
  <div
    ...
    <div class="flex flex-col flex-grow overflow-hidden p-5">
      <div class="flex-grow overflow-y-auto relative">
        <Transition name="fade" mode="out-in">
          <div :key="chatStore.activeTab">
            <ChatList v-if="chatStore.activeTab === 'home'" :chatList="chatStore.chatList" @selectChat="selectChat" />
            <ChatRoom v-if="chatStore.activeTab === 'chat'" :currentChat="chatStore.currentChat" :messages="chatStore.messages" @sendMessage="sendMessage" />
            <div v-if="chatStore.activeTab === 'chatbot'">
    ...
    <div class="flex justify-around items-center h-16 bg-[#f7f4f0] rounded-b-3xl">
      <div @click="setActiveTab('home')" :class="[chatStore.activeTab === 'home' ? 'text-[#e46d0c]' : 'text-[#6E6E6E]']"
        ...
        <i class="pi pi-home transition duration-300 group-hover:text-[#e46d0c]"
          :class="[chatStore.activeTab === 'home' ? 'text-[#e46d0c]' : 'text-[#6E6E6E]']"></i>
        <span class="group-hover:text-[#e46d0c]">홈</span>
      </div>
      <div @click="setActiveTab('chat')" :class="[chatStore.activeTab === 'chat' ? 'text-[#e46d0c]' : 'text-[#6E6E6E]']"
        class="flex flex-col items-center cursor-pointer hover:text-[#e46d0c] transition duration-300 group">
        <i class="pi pi-comment transition duration-300 group-hover:text-[#e46d0c]"
          :class="[chatStore.activeTab === 'chat' ? 'text-[#e46d0c]' : 'text-[#6E6E6E]']"></i>
        <span class="group-hover:text-[#e46d0c]">채팅</span>
      </div>
      <div @click="setActiveTab('chatbot')" :class="[chatStore.activeTab === 'chatbot' ? 'text-[#e46d0c]' : 'text-[#6E6E6E]']"
        class="flex flex-col items-center cursor-pointer hover:text-[#e46d0c] transition duration-300 group">
        <i class="pi pi-microchip-ai transition duration-300 group-hover:text-[#e46d0c]"
          :class="[chatStore.activeTab === 'chatbot' ? 'text-[#e46d0c]' : 'text-[#6E6E6E]']"></i>
